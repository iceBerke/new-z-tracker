...
file written alongside the .npy output. Run instructions are in the class javadoc.
...
- **A single bad point does NOT discard its whole track — only that point is dropped from 3D.** `TrackExportManager.export` used to skip the *entire* track's 3D export if any one detection had a NaN Z; it now keeps the track and exports only the valid-Z subset to 3D (2D is unaffected, since it never depended on Z). 3D is skipped for a track only if too few valid-Z points remain (fewer than `minTrackLength`) after dropping the bad ones — tracked separately as `3Dskipped(insufficientValidZ)`, not lumped in with the whole-track filters (`short`/`noisy`/`invalidXY`). A per-track report is logged on **every** export run (not just when something's wrong) via `TrackExportManager`'s own `[TrackExportManager] ── Per-track report ──` block — one line per track, capped at 50 (`MAX_TRACKS_LOGGED`, same convention as `FrameAligner`'s per-track table), ordered by track id (numeric-aware, same `compareTrackIds` convention). Each line reports whether 2D/3D were written, how many points, and — when points were dropped — a breakdown by reason (e.g. `dropped 1: 1 missing frame`). The same report, **uncapped**, is also written to `export_report.txt` in each export's `outDir` (so per method-combo subfolder when multiple methods run), since the Fiji Log view's cap is for on-screen readability, not data loss. **Dropping a point never renumbers the surviving frame numbers** — 3D's `T` column always holds each kept detection's real `track.frame[i]`, so a dropped point leaves a genuine gap in the frame sequence (e.g. `0,1,3` if frame 2 was dropped) rather than a shift/compaction (never `0,1,2`) — this is intentional and covered by a dedicated test (`export_droppedPointFromMidTrack_leavesGenuineGapInFrameNumbers_notRenumbered`) so it doesn't regress.
...
- **Export filtering** — `TrackExportManager` applies a minimum track length (default 3 frames) and an optional max-Z-std threshold at the whole-track level; 2D and 3D are otherwise independent — 3D drops only the individual NaN-Z detections within a track (see the "single bad point" gotcha above), not the whole track.
