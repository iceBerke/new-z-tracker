...
- **Export filtering** — `TrackExportManager` applies a minimum track length (default 3 frames) and an optional max-Z-std threshold at the whole-track level; 2D and 3D are otherwise independent and gated per-point (see the "single bad point" gotcha above), not the whole track.
