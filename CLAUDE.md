...
- **Frame-number extraction uses the LAST digit run in the filename, not the first — and a filename with no digits at all is rejected, never defaulted.** `TiffStackLoader.extractFrameNumber` (Tool 2) must not just take the first regex match — filenames can contain incidental numbers before the real frame index (e.g. `z_origin_32bit_0007.tif`, where "32" from "32bit" is not the frame number). Taking the first match collapses every file to the same detected frame. **Tool 3 (`TopoJStackLoader`) looks in a different place on purpose** — for filename flexibility it anchors the frame index to the integer the base name (extension stripped) **ends with** (`(\d+)$`): any prefix, any zero-padding width (not hard-coded — `frame7.tif`, `topoj_0007.tif`, `height_map_00000100.tif` all work). **Keep that where-to-look difference**; it is deliberate and neither loader should drift toward the other. What they now agree on (Tool 2 since p10.2, Tool 3 since p9.4) is the *missing* case: each `load` scans every filename up front, before sorting, and throws listing **all** names carrying no frame number. Neither may fall back to frame 0 — `extractFrameNumber` still returns 0 for a digit-less name, but frame 0 is itself a perfectly legitimate frame number, so that value cannot double as an error signal, and letting it through means several such files silently overwrite each other in `frameToIdx` and simply vanish from the stack. The guards are written **separately in each loader, with no shared helper**, same as p10.1's dimension guard.
