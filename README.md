...
The JAR carries its version in the filename — `z-tracker-v4-p10.14.jar` — so you can always tell
which build is installed by looking in Fiji's `plugins/` folder. A change in the second number is
fixes and improvements; a change in the first means the plugin gained a new capability, such as a
new tool or a new kind of input.

The rule itself, and the rest of the patch workflow, is developer material and lives in
`CLAUDE.md`.
